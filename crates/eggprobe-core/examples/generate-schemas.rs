...
fn main() {
    ...
    write_schema(&root.join("schemas/plan-0.4.json"), &plan);
    write_schema(&root.join("schemas/report-0.4.json"), &report);
}

fn write_schema(path: &Path, schema: &schemars::Schema) {
    let mut value: Value = serde_json::from_str(&pretty(schema)).expect("schema JSON");
    value["properties"]["schema_version"] = serde_json::json!({"const": "0.4"});
