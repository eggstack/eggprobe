...
`2afc2e95223d785068af71e6129ae5d47372206a` (Native M001/M002/M004 and historical M005 implementation; M005 now corrective-required)
...
Native corrective C004 planning baseline:

`f82eb8b30e65e003fc95520e37a338f9e80a002c`

Native corrective C004 tested upstream commit:

`040771431b6e0b3f66bb17629636c8a3b4956b45`

Native corrective C004 closure:
...
Native M005 post-closure failure baseline:

`0ce9597aa2acad9a61c45a70c3ffaf56333cf3d5`

Hosted CI run `36138451900` fails the live trace CLI smoke on Ubuntu
(job `108082118033`) and Windows (job `108082118099`) while macOS
(job `108082118186`), Rust 1.89 MSRV (job `108082117794`), and audit
(job `108082117983`) pass. C005 closed this corrective; Windows live
execution is tracked separately by blocked C006.

Native corrective C005 planning baseline:

`1401548`

Native corrective C005 implementation:

`067de09`, `05c332f`, `53226a3`

Native corrective C005 closure:

`plans/closure/native-path-host-diagnostics-corrective/005-status.md`

C005 qualifying hosted run `36156303047` (Ubuntu `108141559598`, macOS
`108141559449`, Windows `108141559394`, MSRV `108141559512`, audit
`108141559181`) is fully green on the `53226a3` head: macOS demonstrates
successful unprivileged loopback, Ubuntu the typed permission-denied path,
and Windows the typed unsupported refusal (fail-safe for the upstream
`trippy-core 0.13.0` privileged-run abort).
...
| Native path/host diagnostics | active | M001/M002/M004 closed; M005 conditionally closed (Linux/macOS live-qualified by C005, Windows live execution refusal qualified); C002 corrective required; C004 ICMP semantic/durable-handoff corrective closed at tested upstream commit `0407714` with PR #9; C003/M003 blocked behind upstream acceptance/merge + publication; C005 closed; C007 ready to qualify Trippy #1793 against the Eggprobe repro; C006 remains blocked on a published fixed backend; M006 blocked independently | `plans/subsystems/native-path-host-diagnostics-roadmap.md` |
...
| Native path/host diagnostics corrective | C005 traceroute privilege and hosted-platform qualification | closed | `plans/implementation/native-path-host-diagnostics-corrective/005-traceroute-privilege-and-hosted-qualification.md` | Platform-aware privilege selection + fail-safe Windows refusal; qualifying hosted run `36156303047` fully green; closure `plans/closure/native-path-host-diagnostics-corrective/005-status.md` |
| Native path/host diagnostics corrective | C007 Trippy #1793 Windows fix qualification/upstream handoff | ready | `plans/implementation/native-path-host-diagnostics-corrective/007-trippy-1793-windows-fix-qualification-and-upstream-handoff.md` | Reproduce 0.13.0 abort in an isolated elevated-Windows harness, prove upstream fix commit `0b3c85b`/current master against the same repro, and durably bind C006 to the verified fix or file a distinct upstream report only if still broken |
| Native path/host diagnostics corrective | C006 Windows trace re-qualification on fixed backend | blocked | `plans/implementation/native-path-host-diagnostics-corrective/006-windows-trace-requalification-on-fixed-backend.md` | C007 evidence + a published `trippy-core`/`trippy-privilege` release containing the verified #1793 fix (or explicitly reconciled equivalent); production Git pins/forks/vendoring not accepted |
...
| Native path/host diagnostics | M005 traceroute/path | conditionally closed | `plans/implementation/native-path-host-diagnostics/005-traceroute-path-diagnostics.md` | Historical closure retained at `plans/closure/native-path-host-diagnostics/005-status.md`; Linux/macOS live dispositions qualified by C005 (`plans/closure/native-path-host-diagnostics-corrective/005-status.md`); Windows live execution blocked on upstream backend fix (C006) with qualified fail-safe refusal in the meantime |
| Native path/host diagnostics corrective | C007 Trippy #1793 Windows fix qualification/upstream handoff | ready | `plans/implementation/native-path-host-diagnostics-corrective/007-trippy-1793-windows-fix-qualification-and-upstream-handoff.md` | Upstream issue #1793 is the matching crash signature; fix commit `0b3c85b` is on master/milestone 0.14.0 but unreleased; C007 must prove it against the Eggprobe repro |
| Native path/host diagnostics corrective | C006 Windows trace re-qualification | blocked | `plans/implementation/native-path-host-diagnostics-corrective/006-windows-trace-requalification-on-fixed-backend.md` | C007 closure plus a published immutable Trippy release containing the C007-qualified fix; Git SHA/fork/vendored patch not accepted |
...
16. ~~Qualify and implement Native M005 traceroute/path~~ — implementation and historical closure landed at `2afc2e9`; later hosted evidence corrected its platform privilege assumptions through C005. Do not treat the historical closure alone as current Windows live-execution qualification.
17. ~~Execute **Native corrective C005** and restore truthful traceroute privilege/platform qualification with a fully green hosted matrix.~~ — done (`067de09`/`05c332f`/`53226a3`, qualifying hosted run `36156303047` fully green, closure `plans/closure/native-path-host-diagnostics-corrective/005-status.md`): macOS unprivileged success, Ubuntu typed permission denial, and Windows typed unsupported refusal are each qualified per-OS dispositions.
18. Execute **Native corrective C007** now: build a durable isolated Windows reproducer, confirm registry `trippy-core 0.13.0` shows the C005 abort, and qualify upstream issue #1793 fix commit `0b3c85b` plus current master against the same repro. File a new upstream issue only if the qualified fix/master still reproduces a distinct failure.
19. Keep **Native corrective C006** blocked until C007 closes and an immutable published Trippy release contains the C007-qualified fix; do not weaken the C005 Windows refusal or adopt a Git/fork/vendor production bridge.
20. Keep M006 blocked until trustworthy PMTU feedback/control exists; traceroute qualification does not solve the PMTU DF/PTB gap.
21. Keep routed datagram/QUIC work in Phase 9; do not reuse the byte-stream Eggress route contract for Phase 8.
