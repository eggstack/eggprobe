...
| Native path/host diagnostics corrective | C007 Trippy #1793 Windows fix qualification/upstream handoff | closed (Case C) | `plans/implementation/native-path-host-diagnostics-corrective/007-trippy-1793-windows-fix-qualification-and-upstream-handoff.md` | Isolated harness proved registry 0.13.0 aborts 3/3 while exact fix commit `0b3c85b` aborts 10/10 and master `c0c758e` aborts 10/10 with the identical signature; distinct-defect upstream issue `fujiapple852/trippy#1881` filed; closure `plans/closure/native-path-host-diagnostics-corrective/007-status.md` |
| Native path/host diagnostics corrective | C006 Windows trace re-qualification on fixed backend | blocked | `plans/implementation/native-path-host-diagnostics-corrective/006-windows-trace-requalification-on-fixed-backend.md` | C007 closed (Case C); requires a published `trippy-core`/`trippy-privilege` release containing the fix for upstream #1881 (a #1793-only release is proven insufficient); production Git pins/forks/vendoring not accepted |
...
| Native path/host diagnostics corrective | C006 Windows trace re-qualification | blocked | `plans/implementation/native-path-host-diagnostics-corrective/006-windows-trace-requalification-on-fixed-backend.md` | C007 closed (Case C: #1793 fix proven insufficient); requires a published immutable Trippy release containing the fix for upstream #1881; Git SHA/fork/vendored patch not accepted |
...
18. ~~Execute **Native corrective C007**~~ — closed as Case C (`plans/closure/native-path-host-diagnostics-corrective/007-status.md`): the durable isolated Windows reproducer shows registry `trippy-core 0.13.0` aborting 3/3 while the exact upstream #1793 fix commit `0b3c85b` aborts 10/10 and current master `c0c758e` aborts 10/10 with the identical signature. Distinct-defect upstream issue `fujiapple852/trippy#1881` filed; no new implementation follows from C007.
19. Keep **Native corrective C006** blocked until an immutable published Trippy release contains the fix for upstream **#1881** (a #1793-only release is proven insufficient by C007); do not weaken the C005 Windows refusal or adopt a Git/fork/vendor production bridge.
